...
class APRPointTransformer(nn.Module):

    # DONE: Step-0:
    # Write DataLoader to mimic loading in PointNet Github; Test if data format is accepted in github PointNet;
    ...
    def __init__(self, config):
        ...
        self.backbone = build_backbone(config) # DONE: Modify Backbone File to Load/Initialize/Build PointNet;   
        ...
        # ...
        # DONE: Add Option to Switch between 2d and 1d conv projection layers (default: 1d) + DONE: Set num_channels[0 & 1], when building backbone; 
        if "proj_t" in config_t.keys():
            proj_t_dict = {"conv1d": nn.Conv1d, "conv2d": nn.Conv2d}
            self.input_proj_t = proj_t_dict[config_t["proj_t"]](self.backbone.num_channels[0], decoder_dim, kernel_size=1)
        
        else:
            self.input_proj_t = nn.Conv1d(self.backbone.num_channels[0], decoder_dim, kernel_size=1)
            
        if "proj_rot" in config_t.keys():
            proj_rot_dict = {"conv1d": nn.Conv1d, "conv2d": nn.Conv2d}
            self.input_proj_rot = proj_rot_dict[config_t["proj_rot"]](self.backbone.num_channels[1], decoder_dim, kernel_size=1)
        
        else:
            self.input_proj_rot = nn.Conv1d(self.backbone.num_channels[1], decoder_dim, kernel_size=1)
    ...
    def forward_transformers(self, data):
        ...
        if isinstance(data, torch.Tensor):
            ...
            samples = data.get('points') # DONE: Insert handeling for 'pointcloud'/'lidar' case;  
            
        else:
            raise NotImplementedError(f"Either 'img' or 'points' values need to be provided in data entries.")
            
        # ...        
        batch_size = samples.shape[0]
        
        # Handle data structures
        if isinstance(samples, (list, torch.Tensor)): # DONE: Add 'pointcloud'/'lidar' case, then convert Samples of Tensors/TensorLists to accepted Input-Format of PointNet;
            samples = nested_tensor_from_tensor_list(samples)
            
        # Extract the reducted_features, the position embedding
        reducted_features, pos = self.backbone(samples)          # DONE: Modify Backbone File to Load/Initialize/Build PointNet;  
                                                                 #     + Make sure to convert Output of Backbone (e.g. in BaseBackbone) NestedTransformers
                                                                 #     + The build_position_encoding' should take output of backbone (i.e., NestedTensor return vector), check;
        ...
        # ...        
        local_proj_t = self.input_proj_t(src_t)
        local_descs_t = self.transformer_t(local_proj_t, mask_t, self.query_embed_t.weight, pos[0])[0][0]
        
        # ...
        # print("CUDA:", torch.cuda.empty_cache(), torch.cuda.memory_summary())
        
        local_proj_rot = self.input_proj_rot(src_rot)
        local_descs_rot = self.transformer_rot(local_proj_rot, mask_rot, self.query_embed_rot.weight, pos[1])[0][0]
    ...
    def forward(self, data):
        ...
        # DONE: Modify how the Data is loaded, in the 'lidar'/'point cloud' case;        

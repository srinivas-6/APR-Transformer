...
class DinoTransPoseNet(nn.Module):

    def __init__(self, config):
        """ Initializes the model.
        """
        super().__init__()
        config["backbone"] = "dino_resnet50"
        ...
        self.input_proj_t = nn.Conv2d(2048, decoder_dim, kernel_size=1)
        self.input_proj_rot = nn.Conv2d(1024, decoder_dim, kernel_size=1)
    ...
    def forward_transformers(self, data):
        ...
        src_t, mask_t = features[1].decompose()
        src_rot, mask_rot = features[0].decompose()
    
        # Run through the transformer to translate to "camera-pose" language
        assert mask_t is not None
        assert mask_rot is not None
        local_descs_t = self.transformer_t(self.input_proj_t(src_t), mask_t, self.query_embed_t.weight, pos[1])[0][0]
        local_descs_rot = self.transformer_rot(self.input_proj_rot(src_rot), mask_rot, self.query_embed_rot.weight, pos[0])[0][0]
